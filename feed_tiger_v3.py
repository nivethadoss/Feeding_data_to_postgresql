...
def main():
    ...
    for i, run in enumerate(results.find(no_cursor_timeout=True)):
        ...
        try:
            ...
            cursor.execute("""
                    INSERT INTO runs (station_id, ptu_id, utb_id, hash_id, start, duration, outcome_val,
                        outcome_msg, mongo_id, year_week, year_month, order_id)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                    RETURNING run_id
                    """,
                       (station_id, ptu_id, utb_id, run['hash_id'], start_dt, duration, int(outcome['val']), outcome['msg'], doc_id,
